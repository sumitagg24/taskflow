...
| 11 | Orphan deploy configs (Dockerfiles / Oracle compose) alongside Vercel config | Medium | **Resolved.** The retired host's config, setup script and runbook are deleted (backend moved to Belmo); Vercel `client` project ignores the rest. Root `vercel.json` adds an opt-in stateless `/api` function (no realtime) + static SPA |
